...
Purpose: Shape the product at the depth the human chooses; a thin seed leaves derivation to later stages, a richer seed is ratified downstream rather than re-derived. A human role, not an agent dispatch.
Inputs: Human idea / discussion; optional concept-examples to copy and customize.
Outputs: concept.md — a seed in the concept-examples template shape: product identity, default baseline stack (Web App / Bootstrap / FastAPI+SQLite), app-appropriate basic seed data, and major capabilities. The template shape is the minimum floor; the human may seed richer content (scope boundaries, precise rules/numbers, architecture/contract sketches). Skipped when the file already exists; otherwise produced via a brainstorming session with the human. The human owns and may edit the file directly.
Boundaries: When authoring on the human's behalf, no invented data model — no entities, fields, status lists, API routes, packages, schemas, or role mechanics the human has not supplied. Detail the human seeds is preserved and ratified downstream, not re-derived. Default stack is a starting template the human may substitute; it is not a mandate.
