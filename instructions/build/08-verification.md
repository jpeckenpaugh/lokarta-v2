...
2. Derive a verification checklist from the approved specifications — each item
   ...
   feature brief, or architecture/API contract). If `concept.md` contains a
   "Definition of Done" or acceptance-criteria section, adopt it as an
   authoritative source for checklist items rather than deriving only from the
   briefs and architecture.
