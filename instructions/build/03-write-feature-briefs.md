...
5. Be explicit and unambiguous so the behavior can be implemented without
   guesswork.
6. **Seed rules are binding.** Transcribe any precise rules, numbers, mechanics,
   or constraints `concept.md` already states into the brief verbatim (per the
   seed contract in `00-README.md`); elaborate only where the seed or feature
   file is silent or underspecified. Where you must fill a gap, make the
   smallest reasonable choice and flag it in your summary rather than silently
   inventing scope.
7. Ensure every feature has a brief; do not silently skip or merge features.
...
- Do NOT flatten, generalize, or drop precise rules or numbers stated in
  `concept.md`; they are binding product intent and are transcribed verbatim.
