...
7. Stay at the capability level. Do not describe behavior, workflows, or
   implementation.
8. **Rich seed: partition, don't flatten.** Where `concept.md` already
   enumerates capabilities with precise rules, numbers, or mechanics, treat them
   as authoritative features rather than re-deriving them, and do not generalize
   or drop detail to satisfy the capability-level rule. Carry precise rules and
   numbers forward for Stage 3 to transcribe into briefs; where a precise rule
   cannot sit at the capability level without being lost, keep it intact in the
   feature file and note that the brief owns it.
...
- Do NOT flatten, weaken, or silently drop precise rules or numbers that
  `concept.md` states; they are product intent and belong to a later stage's
  brief.
