...
- **Seed depth** — how much a seed (`concept.md`, or a scope/bug concept)
  pre-states beyond the template floor. A **thin** seed states only identity,
  stack, seed data, and capabilities; downstream stages derive the rest. A
  **rich** seed also states scope boundaries, precise rules/numbers, or
  architecture/contract sketches. The build pipeline's seed contract governs
  how stages treat seed content at either depth: product intent binds,
  implementation pre-decisions are ratified by the owning stage, conflicts are
  flagged to the human, and only the human edits the seed.
...
5. **Handle the build Stage 1 human role.** The build pipeline's Stage 1
   ...
   **without** inventing data-model or implementation detail the human has not
   supplied. Obtain the human's approval of `concept.md`, then advance to Stage
   2. Seeds may be thin (the template floor) or richer (scope boundaries,
   precise rules/numbers, architecture or contract sketches) at the human's
   choice; whichever depth exists, note it when dispatching downstream stages so
   sub-agents apply the build pipeline's seed contract (see "Seed depth" in the
   Glossary) — ratifying and elaborating seed content rather than re-deriving or
   contradicting it. The no-back-write guard (principle 6) applies to whatever
   seed is produced.
...
7. **Answer at the source, not inline.** When a sub-agent's question reveals a
   ...
   **ratified** by the stage that owns it rather than re-derived, silently
   contradicted, or answered back into the seed by an agent (see the build
   pipeline's seed contract in its `00-README.md`). The human edits `concept.md`
   directly if they wish. Only an instruction-file bug is fixed at the source.
